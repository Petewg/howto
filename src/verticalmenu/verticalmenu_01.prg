...
PROCEDURE Main()
   ...
   DO WHILE lContinue
      ...
      FOR EACH cLine IN aMenu
         hb_DispOutAt( nRow - 1, nCol, Space( nMaxCol ), iif( cLine:__enumIndex() == nMenuIndex, " 08/08", "07/07" ) )
         hb_DispOutAt( nRow, nCol, PadL( PadR( cLine, nMaxCol - 2 ), nMaxCol ), iif( cLine:__enumIndex() == nMenuIndex, "15/08", "00/07" ) )
         hb_DispOutAt( nRow + 1, nCol, Space( nMaxCol ), iif( cLine:__enumIndex() == nMenuIndex, " 08/08", "07/07" ) )
         AAdd( aMenuPos, nRow )
         nRow += 3
      NEXT
